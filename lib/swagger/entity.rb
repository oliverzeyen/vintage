require 'swagger/entity_exposure'
require 'swagger/entity_nesting_exposure'
    ...
    class Entity
      ...
      def to_swagger
        root_exposures.each do |exposure|
          @swagger_type['properties'].merge!(Vintage::Grape::EntityExposure.new(exposure).to_swagger)
        end
      ...
      def sub_types
        root_exposures.each_with_object([]) do |exposure, collection|
          exposure = Vintage::Grape::EntityExposure.new(exposure)
          collection << exposure.sub_type if exposure.sub_type

          exposure.nested_exposures.each do |nested_exposure|
            nested_exposure = Vintage::Grape::EntityExposure.new(nested_exposure)
